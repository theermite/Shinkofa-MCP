...
import { ShareFileSchema, ListPermissionsSchema, DeletePermissionSchema } from "../lib/schemas.js";
...
export function registerSharingTools(server: McpServer, client: DriveClient): void {
  server.tool("drive_share_file", "Share a file or folder (add a permission)", ShareFileSchema.shape, async (p) => {
    return withErrorHandler(async () => {
      ...
      return toolResult(await client.callApi("POST", `/files/${encodeURIComponent(p.fileId)}/permissions`, body, query));
    });
  });
  ...
  server.tool("drive_list_permissions", "List who has access to a file", ListPermissionsSchema.shape, async (p) => {
    return withErrorHandler(async () => {
      ...
      return toolResult(await client.callApi("GET", `/files/${encodeURIComponent(p.fileId)}/permissions`, undefined, query));
    });
  });

  server.tool("drive_remove_permission", "Remove a permission (unshare)", DeletePermissionSchema.shape, async (p) => {
    return withErrorHandler(async () => {
      return toolResult(await client.callApi("DELETE", `/files/${encodeURIComponent(p.fileId)}/permissions/${encodeURIComponent(p.permissionId)}`));
    });
  });
}
