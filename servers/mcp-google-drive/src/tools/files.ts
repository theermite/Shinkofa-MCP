...
import { GOOGLE_WORKSPACE_TYPES, isGoogleWorkspaceType, toolResult, withErrorHandler } from "../lib/utils.js";

export function registerFileTools(server: McpServer, client: DriveClient): void {
  server.tool(
    "drive_list_files",
    "Search and list files in Google Drive. Use q parameter for filtering (e.g. \"name contains 'budget'\", \"mimeType = 'application/vnd.google-apps.folder'\", \"'folderId' in parents\").",
  ...
  server.tool("drive_get_file", "Get file or folder metadata by ID", GetFileSchema.shape, async (p) => {
    return withErrorHandler(async () => {
      ...
      if (p.fields) query.fields = p.fields;
        ...
        query.fields = "id,name,mimeType,modifiedTime,createdTime,size,parents,webViewLink,description,shared,trashed";
  ...
  server.tool(
    "drive_read_file",
    "Read file content. Auto-detects Google Workspace docs (exports to markdown/csv) vs regular files (downloads). Override export format with exportMimeType.",
    ReadFileSchema.shape,
    async (p) => {
      return withErrorHandler(async () => {
        const meta = await client.callApi<{ mimeType: string; name: string }>(
          "GET",
  ...
  server.tool(
    "drive_create_file",
    "Create a file in Google Drive. For folders, use drive_create_folder instead.",
  ...
  server.tool(
    "drive_update_file",
    "Update file metadata and/or content. Use trashed=true to move to trash.",
  ...
  server.tool("drive_move_file", "Move a file to a different folder", MoveFileSchema.shape, async (p) => {
    return withErrorHandler(async () => {
      return toolResult(
        await client.callApi("PATCH", `/files/${encodeURIComponent(p.fileId)}`, undefined, {
  ...
  server.tool(
    "drive_delete_file",
    "Delete a file. Default: moves to trash. Set permanent=true for permanent deletion.",
  ...
  server.tool(
    "drive_export_file",
    "Export a Google Workspace document to a specific format (Docs→PDF/DOCX/Markdown, Sheets→XLSX/CSV, Slides→PDF/PPTX)",
