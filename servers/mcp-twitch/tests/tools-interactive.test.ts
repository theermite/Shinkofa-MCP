...
describe("interactive tools", () => {
  ...
  it("should_create_poll_when_called", async () => {
    ...
    const params = { broadcaster_id: "1", title: "Best game?", choices: [{ title: "A" }, { title: "B" }], duration: 60 };
  ...
  it("should_create_prediction_when_called", async () => {
    ...
    const params = { broadcaster_id: "1", title: "Who wins?", outcomes: [{ title: "A" }, { title: "B" }], prediction_window: 60 };
    await handler(params);
  ...
  it("should_start_raid_when_called", async () => {
    ...
    expect(ctx.callApiSpy).toHaveBeenCalledWith("POST", "/raids", undefined, { from_broadcaster_id: "1", to_broadcaster_id: "2" });
  });
  ...
  it("should_snooze_ad_when_called", async () => {
    ...
    await handler({ broadcaster_id: "1" });
    expect(ctx.callApiSpy).toHaveBeenCalledWith("POST", "/channels/ads/schedule/snooze", undefined, { broadcaster_id: "1" });
  });
  ...
  it("should_return_error_when_TwitchError_thrown", async () => {
    ...
    const handler = ctx.registeredTools.get("create_poll")!;
    const result = await handler({ broadcaster_id: "1", title: "T", choices: [{ title: "A" }, { title: "B" }], duration: 60 }) as { isError: boolean; content: { text: string }[] };
  ...
  it("should_return_rate_limit_error_when_rate_limited", async () => {
    ...
    const handler = ctx.registeredTools.get("start_raid")!;
    const result = await handler({ from_broadcaster_id: "1", to_broadcaster_id: "2" }) as { isError: boolean; content: { text: string }[] };
    expect(result.isError).toBe(true);
