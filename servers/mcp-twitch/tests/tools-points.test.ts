...
describe("points tools", () => {
  it("should_get_custom_rewards_when_called", async () => {
    ...
    expect(ctx.callApiSpy).toHaveBeenCalledWith("GET", "/channel_points/custom_rewards", undefined, { broadcaster_id: "1" });
  });
  ...
  it("should_create_custom_reward_when_called", async () => {
    ...
    expect(ctx.callApiSpy).toHaveBeenCalledWith("POST", "/channel_points/custom_rewards", { title: "Test Reward", cost: 100 }, { broadcaster_id: "1" });
  });
  ...
  it("should_update_custom_reward_when_called", async () => {
    ...
    expect(ctx.callApiSpy).toHaveBeenCalledWith("PATCH", "/channel_points/custom_rewards", { title: "Updated", cost: 200 }, { broadcaster_id: "1", id: "r1" });
  });
  ...
  it("should_delete_custom_reward_when_called", async () => {
    ...
    expect(ctx.callApiSpy).toHaveBeenCalledWith("DELETE", "/channel_points/custom_rewards", undefined, { broadcaster_id: "1", id: "r1" });
  });
  ...
  it("should_get_redemptions_when_called", async () => {
    ...
    expect(ctx.callApiSpy).toHaveBeenCalledWith("GET", "/channel_points/custom_rewards/redemptions", undefined, { broadcaster_id: "1", reward_id: "r1", status: "UNFULFILLED" });
  });
  ...
  it("should_update_redemption_status_when_single_id", async () => {
    ...
    expect(ctx.callApiSpy).toHaveBeenCalledWith("PATCH", "/channel_points/custom_rewards/redemptions", { status: "FULFILLED" }, { broadcaster_id: "1", reward_id: "r1", id: ["red1"] });
  });
  ...
  it("should_update_redemption_status_when_array_ids", async () => {
    ...
    expect(ctx.callApiSpy).toHaveBeenCalledWith("PATCH", "/channel_points/custom_rewards/redemptions", { status: "CANCELED" }, { broadcaster_id: "1", reward_id: "r1", id: ["red1", "red2"] });
  });
  ...
  it("should_return_rate_limit_error_when_rate_limited", async () => {
    ...
    const handler = ctx.registeredTools.get("create_custom_reward")!;
    const result = await handler({ broadcaster_id: "1", title: "T", cost: 1 }) as { isError: boolean; content: { text: string }[] };
    expect(result.isError).toBe(true);
