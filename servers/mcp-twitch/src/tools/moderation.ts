...
import {
  GetBannedUsersSchema, BanUserSchema, UnbanUserSchema,
  GetBlockedTermsSchema, AddBlockedTermSchema, RemoveBlockedTermSchema,
  GetModeratorsSchema, AddRemoveModeratorSchema,
  GetShieldModeSchema, UpdateShieldModeSchema,
  GetAutoModSettingsSchema, UpdateAutoModSettingsSchema,
  WarnUserSchema, GetUnbanRequestsSchema, ResolveUnbanRequestSchema,
} from "../lib/schemas.js";
...
export function registerModerationTools(server: McpServer, client: TwitchClient): void {
  server.tool("get_banned_users", "Get list of banned/timed-out users", GetBannedUsersSchema.shape, async (p) => {
    return toolResult(await client.callApi("GET", "/moderation/banned", undefined, p as Record<string, string | number | boolean | string[] | undefined>));
  });
  ...
  server.tool("unban_user", "Unban a user or remove a timeout", UnbanUserSchema.shape, async (p) => {
    return toolResult(await client.callApi("DELETE", "/moderation/bans", undefined, p as Record<string, string | number | boolean | string[] | undefined>));
  });

  server.tool("get_blocked_terms", "Get blocked terms for a channel", GetBlockedTermsSchema.shape, async (p) => {
    return toolResult(await client.callApi("GET", "/moderation/blocked_terms", undefined, p as Record<string, string | number | boolean | string[] | undefined>));
  });
  ...
  server.tool("add_blocked_term", "Add a blocked term to a channel", AddBlockedTermSchema.shape, async (p) => {
    ...
    return toolResult(await client.callApi("POST", "/moderation/blocked_terms", { text }, { broadcaster_id, moderator_id }));
  });

  server.tool("remove_blocked_term", "Remove a blocked term", RemoveBlockedTermSchema.shape, async (p) => {
    return toolResult(await client.callApi("DELETE", "/moderation/blocked_terms", undefined, p as Record<string, string | number | boolean | string[] | undefined>));
  });

  server.tool("get_moderators", "Get list of channel moderators", GetModeratorsSchema.shape, async (p) => {
    return toolResult(await client.callApi("GET", "/moderation/moderators", undefined, p as Record<string, string | number | boolean | string[] | undefined>));
  });

  server.tool("add_moderator", "Add a channel moderator", AddRemoveModeratorSchema.shape, async (p) => {
    return toolResult(await client.callApi("POST", "/moderation/moderators", undefined, p as Record<string, string | number | boolean | string[] | undefined>));
  });

  server.tool("remove_moderator", "Remove a channel moderator", AddRemoveModeratorSchema.shape, async (p) => {
    return toolResult(await client.callApi("DELETE", "/moderation/moderators", undefined, p as Record<string, string | number | boolean | string[] | undefined>));
  });

  server.tool("get_shield_mode", "Get Shield Mode status", GetShieldModeSchema.shape, async (p) => {
    return toolResult(await client.callApi("GET", "/moderation/shield_mode", undefined, p as Record<string, string | number | boolean | string[] | undefined>));
  });
  ...
  server.tool("update_shield_mode", "Activate or deactivate Shield Mode", UpdateShieldModeSchema.shape, async (p) => {
    ...
    return toolResult(await client.callApi("PUT", "/moderation/shield_mode", { is_active }, { broadcaster_id, moderator_id }));
  });

  server.tool("get_automod_settings", "Get AutoMod settings for a channel", GetAutoModSettingsSchema.shape, async (p) => {
    return toolResult(await client.callApi("GET", "/automod/settings", undefined, p as Record<string, string | number | boolean | string[] | undefined>));
  });
  ...
  server.tool("warn_user", "Warn a user in chat", WarnUserSchema.shape, async (p) => {
    const { broadcaster_id, moderator_id, ...data } = p;
    return toolResult(await client.callApi("POST", "/moderation/warnings", { data }, { broadcaster_id, moderator_id }));
  });

  server.tool("get_unban_requests", "Get unban requests for a channel", GetUnbanRequestsSchema.shape, async (p) => {
    return toolResult(await client.callApi("GET", "/moderation/unban_requests", undefined, p as Record<string, string | number | boolean | string[] | undefined>));
  });
  ...
  server.tool("resolve_unban_request", "Approve or deny an unban request", ResolveUnbanRequestSchema.shape, async (p) => {
    ...
    return toolResult(await client.callApi("PATCH", "/moderation/unban_requests", { unban_request_id, status, resolution_text }, { broadcaster_id, moderator_id }));
  });
}
