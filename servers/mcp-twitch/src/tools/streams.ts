...
import {
  CreateStreamMarkerSchema,
  GetFollowedStreamsSchema,
  GetStreamKeySchema,
  GetStreamMarkersSchema,
  GetStreamsSchema,
} from "../lib/schemas.js";
...
export function registerStreamTools(server: McpServer, client: TwitchClient): void {
  ...
  server.tool(
    ...
    "Get live streams from channels a user follows",
    GetFollowedStreamsSchema.shape,
    async (p) =>
      withErrorHandler(async () =>
        toolResult(
          await client.callApi(
  ...
  server.tool(
    ...
    "Create a stream marker at current timestamp",
    CreateStreamMarkerSchema.shape,
    async (p) =>
      withErrorHandler(async () =>
  ...
  server.tool(
    "get_stream_markers",
    "Get stream markers for a VOD or channel",
    GetStreamMarkersSchema.shape,
    async (p) =>
      withErrorHandler(async () =>
        toolResult(
          await client.callApi(
