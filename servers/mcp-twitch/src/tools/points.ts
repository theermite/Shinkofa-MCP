...
import {
  ...
  DeleteCustomRewardSchema,
  GetCustomRewardsSchema,
  GetRedemptionsSchema,
...
export function registerPointsTools(server: McpServer, client: TwitchClient): void {
  ...
  server.tool(
    ...
    "Fulfill or cancel a channel point redemption",
    UpdateRedemptionStatusSchema.shape,
    async (p) =>
      withErrorHandler(async () => {
        ...
        return toolResult(
          await client.callApi(
            ...
            "/channel_points/custom_rewards/redemptions",
            { status },
            { broadcaster_id, reward_id, id: ids },
          ),
        );
      }),
  );
