...
export function registerPointsTools(server: McpServer, client: TwitchClient): void {
  server.tool("get_custom_rewards", "Get channel point rewards", GetCustomRewardsSchema.shape, async (p) => {
    return toolResult(await client.callApi("GET", "/channel_points/custom_rewards", undefined, p as Record<string, string | number | boolean | string[] | undefined>));
  });
  ...
  server.tool("delete_custom_reward", "Delete a channel point reward", DeleteCustomRewardSchema.shape, async (p) => {
    return toolResult(await client.callApi("DELETE", "/channel_points/custom_rewards", undefined, p as Record<string, string | number | boolean | string[] | undefined>));
  });

  server.tool("get_redemptions", "Get redemptions for a channel point reward", GetRedemptionsSchema.shape, async (p) => {
    return toolResult(await client.callApi("GET", "/channel_points/custom_rewards/redemptions", undefined, p as Record<string, string | number | boolean | string[] | undefined>));
  });
