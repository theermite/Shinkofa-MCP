...
export class DockerClient {
  ...
  constructor(config: DockerClientConfig = {}) {
    this.socketPath =
      config.socketPath ?? (process.platform === "win32" ? "//./pipe/docker_engine" : "/var/run/docker.sock");
        ...
        res.on("end", () => {
          ...
          try {
            ...
            if (res.statusCode && res.statusCode >= 400)
              reject(new DockerError(res.statusCode, (parsed as { message?: string })?.message ?? data));
...
export class DockerError extends Error {
  constructor(
    ...
    super(`Docker error ${status}: ${description}`);
