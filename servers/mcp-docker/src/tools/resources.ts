...
export function registerResourceTools(server: McpServer, client: DockerClient): void {
  ...
  server.tool("list_volumes", "List volumes", { filters: z.string().optional() }, async (p) => {
    return withErrorHandler(() =>
      client.callApi("GET", "/volumes", undefined, p.filters ? { filters: p.filters } : undefined).then(toolResult),
    );
  });
  server.tool(
    "create_volume",
  ...
  server.tool("remove_volume", "Remove a volume", { name: z.string(), force: z.boolean().optional() }, async (p) => {
    return withErrorHandler(() =>
      client
        .callApi("DELETE", `/volumes/${p.name}`, undefined, p.force ? { force: p.force } : undefined)
        .then(toolResult),
    );
  });
  server.tool("prune_volumes", "Remove unused volumes", { filters: z.string().optional() }, async (p) => {
    return withErrorHandler(() =>
      client
        .callApi("POST", "/volumes/prune", undefined, p.filters ? { filters: p.filters } : undefined)
        .then(toolResult),
    );
  });
  // Networks
  server.tool("list_networks", "List networks", { filters: z.string().optional() }, async (p) => {
    return withErrorHandler(() =>
      client.callApi("GET", "/networks", undefined, p.filters ? { filters: p.filters } : undefined).then(toolResult),
    );
  });
  server.tool(
    "create_network",
  ...
  server.tool(
    ...
    { id: z.string().describe("Network ID"), Container: z.string(), EndpointConfig: z.record(z.unknown()).optional() },
    async (p) => {
      const { id, ...body } = p;
  ...
  server.tool(
    ...
    { id: z.string().describe("Network ID"), Container: z.string(), Force: z.boolean().optional() },
    async (p) => {
      const { id, ...body } = p;
  ...
  server.tool("prune_networks", "Remove unused networks", { filters: z.string().optional() }, async (p) => {
    return withErrorHandler(() =>
      client
        .callApi("POST", "/networks/prune", undefined, p.filters ? { filters: p.filters } : undefined)
        .then(toolResult),
    );
  });
}
