...
export function registerSceneTools(server: McpServer, obs: OBSClient): void {
  ...
  server.tool("obs-set-current-scene", "Switch to a scene", {
  ...
  server.tool("obs-create-scene", "Create a new scene", {
    sceneName: z.string(),
  }, async ({ sceneName }) => {
    return withErrorHandler(async () => toolResult(await obs.call("CreateScene", { sceneName })));
  });

  server.tool("obs-remove-scene", "Remove a scene", {
    sceneName: z.string().optional(),
    sceneUuid: z.string().optional(),
  }, async (args) => {
  ...
  server.tool("obs-get-scene-items", "List all items in a scene", {
    sceneName: z.string().optional(),
    sceneUuid: z.string().optional(),
  }, async (args) => {
  ...
  server.tool("obs-get-scene-item-id", "Get scene item ID by source name", {
    sceneName: z.string().optional(),
    sceneUuid: z.string().optional(),
    sourceName: z.string(),
  ...
  server.tool("obs-get-scene-item-transform", "Get transform (position, size, crop) of a scene item", {
    sceneName: z.string().optional(),
    sceneUuid: z.string().optional(),
  ...
  server.tool("obs-set-scene-item-transform", "Set transform of a scene item", {
    sceneName: z.string().optional(),
    sceneUuid: z.string().optional(),
    sceneItemId: z.number(),
  ...
  server.tool("obs-set-scene-item-enabled", "Show/hide a scene item", {
    sceneName: z.string().optional(),
    sceneUuid: z.string().optional(),
    sceneItemId: z.number(),
  ...
  server.tool("obs-create-scene-item", "Add a source to a scene", {
    sceneName: z.string().optional(),
    sceneUuid: z.string().optional(),
  ...
  server.tool("obs-remove-scene-item", "Remove a scene item", {
    sceneName: z.string().optional(),
    sceneUuid: z.string().optional(),
    sceneItemId: z.number(),
  }, async (args) => {
