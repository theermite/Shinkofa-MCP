...
describe("General tool registration", () => {
  it("should_register_12_general_tools", () => {
    ...
    expect(registered).toEqual([
      "obs-get-version", "obs-get-stats", "obs-get-video-settings",
      "obs-set-video-settings", "obs-get-hotkey-list", "obs-trigger-hotkey-by-name",
      "obs-get-profile-list", "obs-set-current-profile",
      "obs-get-scene-collection-list", "obs-set-current-scene-collection",
      "obs-get-studio-mode", "obs-set-studio-mode",
    ]);
...
describe("Scene tool registration", () => {
  it("should_register_12_scene_tools", () => {
    ...
    expect(registered).toEqual([
      ...
      "obs-create-scene", "obs-remove-scene",
      "obs-get-scene-items", "obs-get-scene-item-id", "obs-get-scene-item-transform",
      "obs-set-scene-item-transform", "obs-set-scene-item-enabled",
      "obs-create-scene-item", "obs-remove-scene-item",
    ]);
    expect(registered).toHaveLength(12);
  });
});
...
describe("Input tool registration", () => {
  it("should_register_13_input_tools", () => {
    ...
    expect(registered).toEqual([
      "obs-get-input-list", "obs-get-input-kind-list", "obs-get-input-settings",
      "obs-set-input-settings", "obs-create-input", "obs-remove-input", "obs-set-input-name",
      "obs-get-input-mute", "obs-set-input-mute", "obs-toggle-input-mute",
      "obs-get-input-volume", "obs-set-input-volume", "obs-get-special-inputs",
    ]);
...
describe("Filter tool registration", () => {
  it("should_register_7_filter_tools", () => {
    ...
    expect(registered).toEqual([
      "obs-get-source-filter-list", "obs-get-source-filter", "obs-create-source-filter",
      "obs-set-source-filter-settings", "obs-set-source-filter-enabled",
      "obs-remove-source-filter", "obs-get-filter-kind-list",
    ]);
...
describe("Streaming tool registration", () => {
  it("should_register_18_streaming_tools", () => {
    ...
    expect(registered).toEqual([
      ...
      "obs-get-record-status", "obs-start-record", "obs-stop-record",
      "obs-toggle-record", "obs-toggle-record-pause",
      "obs-get-record-directory", "obs-set-record-directory",
      "obs-get-replay-buffer-status", "obs-start-replay-buffer",
      "obs-stop-replay-buffer", "obs-save-replay-buffer",
      "obs-get-virtual-cam-status", "obs-start-virtual-cam", "obs-stop-virtual-cam",
    ]);
...
describe("Transition tool registration", () => {
  it("should_register_9_transition_tools", () => {
    ...
    expect(registered).toEqual([
      "obs-get-transition-list", "obs-get-current-transition",
      "obs-set-current-transition", "obs-set-transition-duration",
      "obs-get-source-screenshot", "obs-save-source-screenshot",
      "obs-get-media-input-status", "obs-trigger-media-input-action",
