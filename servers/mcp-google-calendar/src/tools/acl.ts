...
export function registerAclTools(server: McpServer, client: GoogleCalendarClient): void {
  server.tool("list_acl", "List access control rules for a calendar", ListAclSchema.shape, async (p) => {
    return withErrorHandler(async () => toolResult(await client.callApi("GET", `/calendars/${encodeURIComponent(p.calendarId)}/acl`)));
  });

  server.tool("get_acl_rule", "Get a specific ACL rule", GetAclRuleSchema.shape, async (p) => {
    return withErrorHandler(async () => toolResult(await client.callApi("GET", `/calendars/${encodeURIComponent(p.calendarId)}/acl/${encodeURIComponent(p.ruleId)}`)));
  });
  ...
  server.tool("update_acl_rule", "Update an ACL rule (change role)", UpdateAclRuleSchema.shape, async (p) => {
    return withErrorHandler(async () => {
      ...
      return toolResult(await client.callApi("PATCH", `/calendars/${encodeURIComponent(calendarId)}/acl/${encodeURIComponent(ruleId)}`, body, sendNotifications !== undefined ? { sendNotifications } : undefined));
    });
  });

  server.tool("delete_acl_rule", "Remove sharing (delete ACL rule)", DeleteAclRuleSchema.shape, async (p) => {
    return withErrorHandler(async () => toolResult(await client.callApi("DELETE", `/calendars/${encodeURIComponent(p.calendarId)}/acl/${encodeURIComponent(p.ruleId)}`)));
  });

  server.tool("query_freebusy", "Query free/busy information for calendars", FreeBusyQuerySchema.shape, async (p) => {
    return withErrorHandler(async () => toolResult(await client.callApi("POST", "/freeBusy", p as Record<string, unknown>)));
  });
  ...
  server.tool("get_setting", "Get a specific user setting", GetSettingSchema.shape, async (p) => {
    return withErrorHandler(async () => toolResult(await client.callApi("GET", `/users/me/settings/${encodeURIComponent(p.setting)}`)));
  });
