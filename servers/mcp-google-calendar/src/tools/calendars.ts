...
export function registerCalendarTools(server: McpServer, client: GoogleCalendarClient): void {
  server.tool("get_calendar", "Get calendar metadata", GetCalendarSchema.shape, async (p) => {
    return withErrorHandler(async () =>
      toolResult(await client.callApi("GET", `/calendars/${encodeURIComponent(p.calendarId)}`)),
    );
  });

  server.tool("create_calendar", "Create a new secondary calendar", CreateCalendarSchema.shape, async (p) => {
    return withErrorHandler(async () =>
      toolResult(await client.callApi("POST", "/calendars", p as Record<string, unknown>)),
    );
  });
  ...
  server.tool("delete_calendar", "Delete a secondary calendar", DeleteCalendarSchema.shape, async (p) => {
    return withErrorHandler(async () =>
      toolResult(await client.callApi("DELETE", `/calendars/${encodeURIComponent(p.calendarId)}`)),
    );
  });

  server.tool("clear_calendar", "Clear all events from a calendar", ClearCalendarSchema.shape, async (p) => {
    return withErrorHandler(async () =>
      toolResult(await client.callApi("POST", `/calendars/${encodeURIComponent(p.calendarId)}/clear`)),
    );
  });
  ...
  server.tool("get_calendar_list_entry", "Get a calendar list entry", GetCalendarListEntrySchema.shape, async (p) => {
    return withErrorHandler(async () =>
      toolResult(await client.callApi("GET", `/users/me/calendarList/${encodeURIComponent(p.calendarId)}`)),
    );
  });
  ...
  server.tool(
    ...
    InsertCalendarListSchema.shape,
    async (p) => {
      return withErrorHandler(async () =>
        toolResult(await client.callApi("POST", "/users/me/calendarList", p as Record<string, unknown>)),
      );
    },
  );
  ...
  server.tool(
    ...
    "Update a calendar list entry (colors, visibility, reminders)",
    UpdateCalendarListSchema.shape,
    async (p) => {
      return withErrorHandler(async () => {
        const { calendarId, ...body } = p;
        return toolResult(
          await client.callApi("PATCH", `/users/me/calendarList/${encodeURIComponent(calendarId)}`, body),
        );
      });
    },
  );
  ...
  server.tool(
    ...
    DeleteCalendarListSchema.shape,
    async (p) => {
      return withErrorHandler(async () =>
        toolResult(await client.callApi("DELETE", `/users/me/calendarList/${encodeURIComponent(p.calendarId)}`)),
      );
    },
  );
}
