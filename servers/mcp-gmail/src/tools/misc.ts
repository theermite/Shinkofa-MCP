...
export function registerMiscTools(server: McpServer, client: GmailClient): void {
  server.tool("list_history", "List mailbox history changes since a given historyId", ListHistorySchema.shape, async (p) => {
    ...
    return toolResult(await client.callApi("GET", `/users/${userId}/history`, undefined, query as Record<string, string | number | boolean | undefined>));
  });

  server.tool("get_profile", "Get Gmail profile (email, messages total, threads total, history ID)", GetProfileSchema.shape, async (p) => {
    return toolResult(await client.callApi("GET", `/users/${p.userId}/profile`));
  });

  server.tool("get_vacation_settings", "Get vacation auto-reply settings", GetVacationSchema.shape, async (p) => {
    return toolResult(await client.callApi("GET", `/users/${p.userId}/settings/vacation`));
  });

  server.tool("update_vacation_settings", "Update vacation auto-reply settings", UpdateVacationSchema.shape, async (p) => {
    const { userId, ...body } = p;
  ...
  server.tool("stop_watch", "Stop push notifications", StopWatchSchema.shape, async (p) => {
    return toolResult(await client.callApi("POST", `/users/${p.userId}/stop`));
  });
