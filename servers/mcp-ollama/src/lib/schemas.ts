...
export const ModelName = z.string().min(1).describe("Model name (e.g. 'llama3', 'qwen3:8b', 'gemma4:26b')");
...
export const ModelOptions = z
  .object({
    ...
    num_predict: z.number().optional().describe("Max tokens to generate (-1 = infinite)"),
    stop: z.array(z.string()).optional().describe("Stop sequences"),
    num_ctx: z.number().optional().describe("Context window size (default: 2048)"),
...
export const ChatMessageSchema = z.object({
  ...
  tool_calls: z.array(z.record(z.unknown())).optional().describe("Tool calls from assistant"),
});

export const ChatSchema = z.object({
  model: ModelName,
  messages: z.array(ChatMessageSchema).min(1).describe("Conversation messages"),
  tools: z.array(z.record(z.unknown())).optional().describe("Tool/function definitions for function calling"),
  format: Format,
  options: ModelOptions,
  keep_alive: KeepAlive,
  think: z.boolean().optional().describe("Enable thinking/reasoning"),
...
export const CreateModelSchema = z.object({
  model: ModelName,
  modelfile: z.string().optional().describe("Modelfile content (FROM, PARAMETER, SYSTEM, etc.)"),
...
export const CheckBlobSchema = z.object({
  digest: z.string().min(1).describe("SHA256 digest (sha256:<hex>)"),
});

// ── Raw ──

export const RawApiCallSchema = z.object({
  method: z.enum(["GET", "POST", "DELETE", "HEAD", "PUT"]).describe("HTTP method"),
