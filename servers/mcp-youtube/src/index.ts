...
async function main(): Promise<void> {
  const client = new YouTubeClient({ apiKey: process.env["YOUTUBE_API_KEY"], accessToken: process.env["GOOGLE_ACCESS_TOKEN"], apiBaseUrl: process.env["YOUTUBE_API_BASE_URL"], timeoutMs: process.env["YOUTUBE_TIMEOUT_MS"] ? parseInt(process.env["YOUTUBE_TIMEOUT_MS"], 10) : undefined });
