...
export function registerCommentTools(server: McpServer, client: YouTubeClient): void {
  server.tool("list_comments", "List comment replies", CommentsListSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("GET", "/comments", undefined, p as Record<string, string | number | boolean | string[] | undefined>)))
  );
  server.tool("reply_to_comment", "Reply to a comment", CommentsInsertSchema.shape, async (p) =>
    withErrorHandler(async () => { const { part, parentId, textOriginal } = p; return toolResult(await client.callApi("POST", "/comments", { snippet: { parentId, textOriginal } }, { part: part ?? "snippet" })); })
  );
  server.tool("update_comment", "Update a comment", CommentsUpdateSchema.shape, async (p) =>
    withErrorHandler(async () => { const { part, id, textOriginal } = p; return toolResult(await client.callApi("PUT", "/comments", { id, snippet: { textOriginal } }, { part: part ?? "snippet" })); })
  );
  ...
  server.tool("set_comment_moderation", "Set moderation status on comments", CommentsSetModerationStatusSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("POST", "/comments/setModerationStatus", undefined, p as Record<string, string | number | boolean | string[] | undefined>)))
  );
  server.tool("list_comment_threads", "List top-level comment threads on a video/channel", CommentThreadsListSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("GET", "/commentThreads", undefined, p as Record<string, string | number | boolean | string[] | undefined>)))
  );
