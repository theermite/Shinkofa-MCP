...
export const CropSchema = z.object({
  ...
  x: z.number().optional().describe("X offset"), y: z.number().optional().describe("Y offset"),
...
export const RotateSchema = z.object({
  ...
  background: z.string().optional().describe("Background color for uncovered areas (e.g. 'white', '#FF0000', 'transparent')"),
});
...
export const SharpenSchema = z.object({
  input: z.string(), output: z.string(),
  radius: z.number().min(0), sigma: z.number().min(0),
});

export const TextOverlaySchema = z.object({
  input: z.string(), output: z.string(),
  text: z.string(), gravity: z.enum(["NorthWest", "North", "NorthEast", "West", "Center", "East", "SouthWest", "South", "SouthEast"]).optional(),
  font: z.string().optional(), pointsize: z.number().optional(),
  fill: z.string().optional().describe("Text color"), stroke: z.string().optional(),
  strokewidth: z.number().optional(),
});

export const CompositeSchema = z.object({
  base: z.string().describe("Base image path"), overlay: z.string().describe("Overlay image path"),
  output: z.string(), gravity: z.enum(["NorthWest", "North", "NorthEast", "West", "Center", "East", "SouthWest", "South", "SouthEast"]).optional(),
...
export const BatchConvertSchema = z.object({
  ...
  output_dir: z.string(), output_format: z.string().describe("Target format (webp, avif, jpg, etc.)"),
  resize: z.string().optional(), quality: z.number().min(1).max(100).optional(),
});

export const CreateGifSchema = z.object({
  input_pattern: z.string().describe("Input frames pattern (e.g. 'frame_*.png')"),
  output: z.string(), delay: z.number().describe("Delay between frames in centiseconds"),
...
export const CreateSpriteSheetSchema = z.object({
  input_pattern: z.string(), output: z.string(),
...
export const BorderSchema = z.object({
  input: z.string(), output: z.string(),
  width: z.number(), color: z.string().optional().describe("Border color (default: black)"),
});

export const ShadowSchema = z.object({
  input: z.string(), output: z.string(),
  opacity: z.number().min(0).max(100).optional(),
  sigma: z.number().optional(), x_offset: z.number().optional(), y_offset: z.number().optional(),
