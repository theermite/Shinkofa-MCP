...
export function registerAdvancedTools(server: McpServer, runner: ImageMagickRunner): void {
  ...
  server.tool(
    "composite_images",
    "Overlay one image on another (watermark, logo, etc.)",
  ...
  server.tool(
    "batch_convert",
    "Convert multiple images (glob pattern) to another format",
    BatchConvertSchema.shape,
    async (p) => {
      return withErrorHandler(async () => {
        ...
        const r = await runner.run(args);
        return r.exitCode === 0
          ? toolResult({ output_dir: p.output_dir, format: p.output_format })
          : toolError(r.stderr);
      });
    },
  );
  ...
  server.tool("raw_magick", "Run any ImageMagick command directly", RawMagickSchema.shape, async (p) => {
    return withErrorHandler(async () => {
      ...
      return r.exitCode === 0
        ? toolResult({ stdout: r.stdout.trim(), stderr: r.stderr.trim() })
        : toolError(`Exit ${r.exitCode}: ${r.stderr}`);
    });
  });
