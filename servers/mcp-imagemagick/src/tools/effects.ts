...
import { BlurSchema, BorderSchema, ColorAdjustSchema, ShadowSchema, SharpenSchema } from "../lib/schemas.js";
...
export function registerEffectTools(server: McpServer, runner: ImageMagickRunner): void {
  ...
  server.tool(
    "adjust_colors",
    "Adjust brightness, contrast, saturation, or colorspace",
    ColorAdjustSchema.shape,
    async (p) => {
      return withErrorHandler(async () => {
