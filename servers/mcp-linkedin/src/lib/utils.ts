  ...
  try {
    ...
    if (error instanceof LinkedInError) {
      const extra = error.isRateLimited
        ? " (rate limited — 150 req/day/member)"
        : "";
      return toolError(`LinkedIn error ${error.status}: ${error.detail}${extra}`);
    }
    if (error instanceof Error) {
      if (error.name === "AbortError") return toolError("Request timed out");
      if (error.name === "SyntaxError")
        return toolError("Invalid response from LinkedIn (non-JSON)");
