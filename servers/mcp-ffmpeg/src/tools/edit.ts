...
import { TrimSchema, ResizeSchema, SpeedSchema, ConcatSchema } from "../lib/schemas.js";
...
export function registerEditTools(server: McpServer, config: ExecutorConfig): void {
  server.tool("trim", "Cut a segment from a media file by start/end time or duration", TrimSchema.shape, async (params) => {
  ...
  server.tool("resize", "Resize/scale a video (with optional letterboxing and GPU acceleration)", ResizeSchema.shape, async (params) => {
    return withErrorHandler(async () => {
      await validateInputFile(params.input);
      const args = ["-y"];
      ...
      if (params.gpu) {
        ...
        args.push("-vf", `scale=${params.width}:${params.height}:force_original_aspect_ratio=decrease:flags=${params.algorithm},pad=${params.width}:${params.height}:(ow-iw)/2:(oh-ih)/2:black`);
      } else {
  ...
  server.tool("concat", "Concatenate multiple media files (with optional crossfade transitions)", ConcatSchema.shape, async (params) => {
    return withErrorHandler(async () => {
      ...
      if (params.copy && !params.transition) {
        ...
        try {
          return await runFfmpeg(config, ["-y", "-f", "concat", "-safe", "0", "-i", listPath, "-c", "copy", params.output]);
        } finally {
        ...
        if (params.transition && n === 2) {
          ...
          args.push("-filter_complex", `[0:v][1:v]xfade=transition=${params.transition}:duration=${dur}:offset=4[v];[0:a][1:a]acrossfade=d=${dur}[a]`, "-map", "[v]", "-map", "[a]");
        } else {
