...
export function registerRawTools(server: McpServer, config: ExecutorConfig): void {
  server.tool("raw_ffmpeg", "Execute any FFmpeg command with custom arguments. Full access to all FFmpeg capabilities. Arguments are passed directly (without 'ffmpeg' prefix).", RawFfmpegSchema.shape, async (params) => {
  ...
  server.tool("raw_ffprobe", "Execute any FFprobe command with custom arguments. Full access to all analysis capabilities. Arguments are passed directly (without 'ffprobe' prefix).", RawFfprobeSchema.shape, async (params) => {
    return withErrorHandler(async () => {
