...
import { ChromakeySchema, ComposeSchema, DrawtextSchema, WatermarkSchema } from "../lib/schemas.js";
        ...
        switch (params.layout) {
          ...
          case "grid": {
            ...
            if (n === 4) {
              args.push(
                "-filter_complex",
                `[0:v][1:v]hstack[top];[2:v][3:v]hstack[bot];[top][bot]vstack[v]`,
                "-map",
                "[v]",
              );
            } else {
              ...
              args.push(
                "-filter_complex",
                `${streams}xstack=inputs=${n}:layout=${buildGridLayout(n, cols)}[v]`,
                "-map",
                "[v]",
              );
            }
