...
export function registerWorkflowTools(server: McpServer, client: N8nClient): void {
  server.tool(
    ...
    async (p) => {
      return withErrorHandler(() =>
        client
          .callApi("GET", "/workflows", undefined, p as Record<string, string | number | boolean | undefined>)
          .then(toolResult),
      );
    },
  );
  ...
  server.tool(
    ...
    async (p) => {
      return withErrorHandler(() =>
        client.callApi("POST", "/workflows", p as Record<string, unknown>).then(toolResult),
      );
    },
  );
  server.tool(
  ...
  server.tool(
    ...
    { id: z.string(), tags: z.array(z.object({ id: z.string() })) },
    async (p) => {
      return withErrorHandler(() =>
        client.callApi("PUT", `/workflows/${p.id}/tags`, p.tags as unknown as Record<string, unknown>).then(toolResult),
      );
    },
  );
}
