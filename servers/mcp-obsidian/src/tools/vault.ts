...
import type { ObsidianClient } from "../lib/client.js";
import {
...
export function registerVaultTools(server: McpServer, client: ObsidianClient): void {
    ...
    withErrorHandler(async () =>
      toolResult(
        await client.callApi(
          "GET",
          `/vault/${encodeURIComponent(p.path)}`,
          undefined,
  ...
  server.tool("delete_note", "Delete a note from the vault", DeleteNoteSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("DELETE", `/vault/${encodeURIComponent(p.path)}`))),
  );
  ...
  server.tool("search_vault_jsonlogic", "Search notes with JsonLogic query", SearchJsonLogicSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("POST", "/search/", p.query))),
  );
  ...
  server.tool("update_active_file", "Replace content of the active file", UpdateActiveFileSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("PUT", "/active/", p.content))),
  );

  server.tool("append_to_active_file", "Append content to the active file", AppendActiveFileSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("POST", "/active/", p.content))),
  );

  server.tool("list_commands", "List all available Obsidian commands", ListCommandsSchema.shape, async () =>
    withErrorHandler(async () => toolResult(await client.callApi("GET", "/commands/"))),
  );
  ...
  server.tool("open_note", "Open a note in Obsidian", OpenNoteSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(
        await client.callApi(
          "POST",
          `/open/${encodeURIComponent(p.path)}`,
          p.newLeaf !== undefined ? { newLeaf: p.newLeaf } : undefined,
        ),
      ),
    ),
  );
  ...
  server.tool(
    ...
    async (p) =>
      withErrorHandler(async () =>
        toolResult(
          await client.callApi("GET", `/periodic/${p.period}/`, undefined, "application/vnd.olrapi.note+json"),
        ),
      ),
  );

  server.tool("get_status", "Get Obsidian vault status and server info", GetStatusSchema.shape, async () =>
    withErrorHandler(async () => toolResult(await client.callApi("GET", "/"))),
  );

  server.tool("raw_api_call", "Call any Obsidian Local REST API endpoint directly", RawApiCallSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi(p.method, p.path, p.body ?? undefined, p.accept))),
  );
}
