...
import { GetInviteSchema, DeleteInviteSchema } from "../lib/schemas.js";
...
export function registerInviteTools(server: McpServer, client: DiscordClient): void {
  server.tool("get_invite", "Get an invite by code", GetInviteSchema.shape, async (p) =>
    withErrorHandler(async () => {
      ...
      return toolResult(await client.callApi("GET", `/invites/${invite_code}`, undefined, query as Record<string, string | number | boolean | undefined>));
    })
  );

  server.tool("delete_invite", "Delete/revoke an invite", DeleteInviteSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("DELETE", `/invites/${p.invite_code}`, undefined, undefined, p.reason)))
  );
}
