...
export function registerEventTools(server: McpServer, client: DiscordClient): void {
  server.tool("list_scheduled_events", "List all scheduled events in a guild", ListEventsSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("GET", `/guilds/${p.guild_id}/scheduled-events`, undefined, { with_user_count: p.with_user_count })))
  );
  ...
  server.tool("get_scheduled_event", "Get a scheduled event by ID", GetEventSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("GET", `/guilds/${p.guild_id}/scheduled-events/${p.event_id}`, undefined, { with_user_count: p.with_user_count })))
  );
  ...
  server.tool("delete_scheduled_event", "Delete a scheduled event", DeleteEventSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("DELETE", `/guilds/${p.guild_id}/scheduled-events/${p.event_id}`)))
  );
