...
export function registerEventTools(server: McpServer, client: DiscordClient): void {
  server.tool("list_scheduled_events", "List all scheduled events in a guild", ListEventsSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(
        await client.callApi("GET", `/guilds/${p.guild_id}/scheduled-events`, undefined, {
          with_user_count: p.with_user_count,
        }),
      ),
    ),
  );
  ...
  server.tool(
    ...
    "Create a scheduled event (voice, stage, or external)",
    CreateEventSchema.shape,
    async (p) =>
      withErrorHandler(async () => {
        const { guild_id, reason, ...body } = p;
        return toolResult(
          await client.callApi("POST", `/guilds/${guild_id}/scheduled-events`, body, undefined, reason),
        );
      }),
  );
  ...
  server.tool("get_scheduled_event", "Get a scheduled event by ID", GetEventSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(
        await client.callApi("GET", `/guilds/${p.guild_id}/scheduled-events/${p.event_id}`, undefined, {
          with_user_count: p.with_user_count,
        }),
      ),
    ),
  ...
  server.tool("modify_scheduled_event", "Modify a scheduled event", ModifyEventSchema.shape, async (p) =>
    withErrorHandler(async () => {
      ...
      return toolResult(
        await client.callApi("PATCH", `/guilds/${guild_id}/scheduled-events/${event_id}`, body, undefined, reason),
      );
    }),
  );
  ...
  server.tool("delete_scheduled_event", "Delete a scheduled event", DeleteEventSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(await client.callApi("DELETE", `/guilds/${p.guild_id}/scheduled-events/${p.event_id}`)),
    ),
  );
