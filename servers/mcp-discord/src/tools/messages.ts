...
export function registerMessageTools(server: McpServer, client: DiscordClient): void {
  ...
  server.tool("get_message", "Get a specific message", GetMessageSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("GET", `/channels/${p.channel_id}/messages/${p.message_id}`)))
  );
  ...
  server.tool("delete_message", "Delete a message", DeleteMessageSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("DELETE", `/channels/${p.channel_id}/messages/${p.message_id}`, undefined, undefined, p.reason)))
  );
  ...
  server.tool("crosspost_message", "Crosspost a message in an announcement channel to following channels", CrosspostMessageSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("POST", `/channels/${p.channel_id}/messages/${p.message_id}/crosspost`)))
  );

  server.tool("add_reaction", "Add a reaction to a message", AddReactionSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("PUT", `/channels/${p.channel_id}/messages/${p.message_id}/reactions/${encodeURIComponent(p.emoji)}/@me`)))
  );
  ...
  server.tool("pin_message", "Pin a message in a channel (max 50 pins)", PinMessageSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("PUT", `/channels/${p.channel_id}/pins/${p.message_id}`, undefined, undefined, p.reason)))
  );

  server.tool("unpin_message", "Unpin a message", PinMessageSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("DELETE", `/channels/${p.channel_id}/pins/${p.message_id}`, undefined, undefined, p.reason)))
  );

  server.tool("get_pinned_messages", "Get all pinned messages in a channel", GetPinnedMessagesSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("GET", `/channels/${p.channel_id}/pins`)))
  );
}
