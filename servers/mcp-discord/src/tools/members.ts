...
export function registerMemberTools(server: McpServer, client: DiscordClient): void {
  server.tool("get_member", "Get a guild member", GetMemberSchema.shape, async (p) => {
    return toolResult(await client.callApi("GET", `/guilds/${p.guild_id}/members/${p.user_id}`));
  });
  ...
  server.tool("kick_member", "Kick a member from the guild", KickMemberSchema.shape, async (p) => {
    return toolResult(await client.callApi("DELETE", `/guilds/${p.guild_id}/members/${p.user_id}`, undefined, undefined, p.reason));
  });
  ...
  server.tool("unban_member", "Unban a user", UnbanMemberSchema.shape, async (p) => {
    return toolResult(await client.callApi("DELETE", `/guilds/${p.guild_id}/bans/${p.user_id}`, undefined, undefined, p.reason));
  });
  ...
  server.tool("get_ban", "Get ban info for a specific user", GetBanSchema.shape, async (p) => {
    return toolResult(await client.callApi("GET", `/guilds/${p.guild_id}/bans/${p.user_id}`));
  });
  ...
  server.tool("get_roles", "List all roles in a guild", GetRolesSchema.shape, async (p) => {
    return toolResult(await client.callApi("GET", `/guilds/${p.guild_id}/roles`));
  });
  ...
  server.tool("delete_role", "Delete a role", DeleteRoleSchema.shape, async (p) => {
    return toolResult(await client.callApi("DELETE", `/guilds/${p.guild_id}/roles/${p.role_id}`, undefined, undefined, p.reason));
  });

  server.tool("modify_role_positions", "Reorder roles", ModifyRolePositionsSchema.shape, async (p) => {
    return toolResult(await client.callApi("PATCH", `/guilds/${p.guild_id}/roles`, { roles: p.roles } as unknown as Record<string, unknown>));
  });

  server.tool("assign_role", "Add a role to a guild member", AssignRoleSchema.shape, async (p) => {
    return toolResult(await client.callApi("PUT", `/guilds/${p.guild_id}/members/${p.user_id}/roles/${p.role_id}`, undefined, undefined, p.reason));
  });

  server.tool("remove_role", "Remove a role from a guild member", RemoveRoleSchema.shape, async (p) => {
    return toolResult(await client.callApi("DELETE", `/guilds/${p.guild_id}/members/${p.user_id}/roles/${p.role_id}`, undefined, undefined, p.reason));
  });
}
