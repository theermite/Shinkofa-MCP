...
export function registerMemberTools(server: McpServer, client: DiscordClient): void {
  ...
  server.tool(
    "modify_member",
    "Modify a guild member (nickname, roles, mute, deaf, timeout)",
    ModifyMemberSchema.shape,
    async (p) =>
      withErrorHandler(async () => {
        const { guild_id, user_id, reason, ...body } = p;
        return toolResult(
          await client.callApi("PATCH", `/guilds/${guild_id}/members/${user_id}`, body, undefined, reason),
        );
      }),
  );
  ...
  server.tool("kick_member", "Kick a member from the guild", KickMemberSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(
        await client.callApi("DELETE", `/guilds/${p.guild_id}/members/${p.user_id}`, undefined, undefined, p.reason),
      ),
    ),
  );
  ...
  server.tool("unban_member", "Unban a user", UnbanMemberSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(
        await client.callApi("DELETE", `/guilds/${p.guild_id}/bans/${p.user_id}`, undefined, undefined, p.reason),
      ),
    ),
  );
  ...
  server.tool("delete_role", "Delete a role", DeleteRoleSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(
        await client.callApi("DELETE", `/guilds/${p.guild_id}/roles/${p.role_id}`, undefined, undefined, p.reason),
      ),
    ),
  );
  ...
  server.tool("modify_role_positions", "Reorder roles", ModifyRolePositionsSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(
        await client.callApi("PATCH", `/guilds/${p.guild_id}/roles`, { roles: p.roles } as unknown as Record<
          string,
          unknown
        >),
      ),
  ...
  server.tool("assign_role", "Add a role to a guild member", AssignRoleSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(
        await client.callApi(
          "PUT",
          `/guilds/${p.guild_id}/members/${p.user_id}/roles/${p.role_id}`,
          undefined,
          undefined,
          p.reason,
        ),
