...
import {
  ...
  GetChannelSchema,
  JoinLeaveThreadSchema,
  ListArchivedThreadsSchema,
...
export function registerChannelTools(server: McpServer, client: DiscordClient): void {
  server.tool("get_channel", "Get a channel by ID", GetChannelSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("GET", `/channels/${p.channel_id}`))),
  );

  server.tool(
    "modify_channel",
    "Modify a channel's settings (name, topic, slowmode, etc.)",
  ...
  server.tool("delete_channel", "Delete a channel or close a DM", DeleteChannelSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(await client.callApi("DELETE", `/channels/${p.channel_id}`, undefined, undefined, p.reason)),
    ),
  );

  server.tool(
    "edit_channel_permissions",
    "Edit permission overwrites for a role or user in a channel",
    EditPermissionsSchema.shape,
    async (p) =>
      withErrorHandler(async () => {
        const { channel_id, overwrite_id, reason, ...body } = p;
        return toolResult(
          await client.callApi("PUT", `/channels/${channel_id}/permissions/${overwrite_id}`, body, undefined, reason),
        );
      }),
  );
  ...
  server.tool(
    ...
    "Delete permission overwrites for a role or user",
    DeletePermissionsSchema.shape,
    async (p) =>
      withErrorHandler(async () =>
        toolResult(
          await client.callApi(
            "DELETE",
            `/channels/${p.channel_id}/permissions/${p.overwrite_id}`,
            undefined,
            undefined,
            p.reason,
          ),
  ...
  server.tool("trigger_typing", "Show typing indicator in a channel", TriggerTypingSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("POST", `/channels/${p.channel_id}/typing`))),
  );
  ...
  server.tool("get_channel_invites", "Get all invites for a channel", GetChannelInvitesSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("GET", `/channels/${p.channel_id}/invites`))),
  );

  server.tool(
    "follow_announcement_channel",
    "Follow an announcement channel to receive crossposted messages",
    FollowAnnouncementChannelSchema.shape,
    async (p) =>
      withErrorHandler(async () =>
        toolResult(
          await client.callApi("POST", `/channels/${p.channel_id}/followers`, {
            webhook_channel_id: p.webhook_channel_id,
          }),
        ),
      ),
  );
  ...
  server.tool(
    ...
    CreateThreadFromMessageSchema.shape,
    async (p) =>
      withErrorHandler(async () => {
  ...
  server.tool("join_thread", "Join a thread", JoinLeaveThreadSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(await client.callApi("PUT", `/channels/${p.channel_id}/thread-members/@me`)),
    ),
  );

  server.tool("leave_thread", "Leave a thread", JoinLeaveThreadSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(await client.callApi("DELETE", `/channels/${p.channel_id}/thread-members/@me`)),
    ),
  );

  server.tool("add_thread_member", "Add a member to a thread", AddRemoveThreadMemberSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(await client.callApi("PUT", `/channels/${p.channel_id}/thread-members/${p.user_id}`)),
    ),
  );

  server.tool("remove_thread_member", "Remove a member from a thread", AddRemoveThreadMemberSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(await client.callApi("DELETE", `/channels/${p.channel_id}/thread-members/${p.user_id}`)),
    ),
  );
  ...
  server.tool(
    ...
    ListArchivedThreadsSchema.shape,
    async (p) =>
      withErrorHandler(async () => {
        ...
        return toolResult(
          await client.callApi(
            "GET",
            `/channels/${channel_id}/threads/archived/${type}`,
            undefined,
            query as Record<string, string | number | boolean | undefined>,
          ),
        );
