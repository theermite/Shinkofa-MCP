...
export function registerChannelTools(server: McpServer, client: DiscordClient): void {
  ...
  server.tool("delete_channel", "Delete a channel or close a DM", DeleteChannelSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("DELETE", `/channels/${p.channel_id}`, undefined, undefined, p.reason)))
  );
  ...
  server.tool("delete_channel_permissions", "Delete permission overwrites for a role or user", DeletePermissionsSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("DELETE", `/channels/${p.channel_id}/permissions/${p.overwrite_id}`, undefined, undefined, p.reason)))
  );
  ...
  server.tool("follow_announcement_channel", "Follow an announcement channel to receive crossposted messages", FollowAnnouncementChannelSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("POST", `/channels/${p.channel_id}/followers`, { webhook_channel_id: p.webhook_channel_id })))
  );
  ...
  server.tool("join_thread", "Join a thread", JoinLeaveThreadSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("PUT", `/channels/${p.channel_id}/thread-members/@me`)))
  );

  server.tool("leave_thread", "Leave a thread", JoinLeaveThreadSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("DELETE", `/channels/${p.channel_id}/thread-members/@me`)))
  );

  server.tool("add_thread_member", "Add a member to a thread", AddRemoveThreadMemberSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("PUT", `/channels/${p.channel_id}/thread-members/${p.user_id}`)))
  );

  server.tool("remove_thread_member", "Remove a member from a thread", AddRemoveThreadMemberSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("DELETE", `/channels/${p.channel_id}/thread-members/${p.user_id}`)))
  );
