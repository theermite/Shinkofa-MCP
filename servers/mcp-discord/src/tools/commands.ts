...
export function registerCommandTools(server: McpServer, client: DiscordClient): void {
  server.tool(
    "list_commands",
    "List application commands (global or guild-specific)",
  ...
  server.tool(
    "create_command",
    "Create an application command (global or guild-specific)",
    CreateCommandSchema.shape,
    async (p) =>
      withErrorHandler(async () => {
  ...
  server.tool(
    ...
    "Overwrite all application commands at once",
    BulkOverwriteCommandsSchema.shape,
    async (p) =>
      withErrorHandler(async () => {
        const path = p.guild_id
          ? `/applications/${p.application_id}/guilds/${p.guild_id}/commands`
