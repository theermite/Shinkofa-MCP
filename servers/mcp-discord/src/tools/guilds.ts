...
export function registerGuildTools(server: McpServer, client: DiscordClient): void {
  server.tool("get_guild", "Get a guild (server) by ID", GetGuildSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(await client.callApi("GET", `/guilds/${p.guild_id}`, undefined, { with_counts: p.with_counts })),
    ),
  );

  server.tool(
    "modify_guild",
    "Modify guild settings (name, verification, notifications, etc.)",
    ModifyGuildSchema.shape,
    async (p) =>
      withErrorHandler(async () => {
  ...
  server.tool(
    ...
    ModifyChannelPositionsSchema.shape,
    async (p) =>
      withErrorHandler(async () =>
        toolResult(
          await client.callApi("PATCH", `/guilds/${p.guild_id}/channels`, { channels: p.channels } as unknown as Record<
            string,
            unknown
          >),
        ),
  ...
  server.tool(
    "modify_onboarding",
    "Modify guild onboarding (prompts, default channels)",
    ModifyOnboardingSchema.shape,
    async (p) =>
      withErrorHandler(async () => {
        const { guild_id, reason, ...body } = p;
