...
describe("Subscription tools — callbacks", () => {
  ...
  it("should_update_subscription_with_encoded_id", async () => {
    ...
    expect(callApiSpy).toHaveBeenCalledWith("POST", "/subscriptions/sub_123", { cancel_at_period_end: true });
  });
  ...
  it("should_get_subscription_with_expand", async () => {
    ...
    expect(callApiSpy).toHaveBeenCalledWith("GET", "/subscriptions/sub_123", { expand: ["latest_invoice"] });
  });
  ...
  it("should_cancel_subscription_via_DELETE", async () => {
    ...
    expect(callApiSpy).toHaveBeenCalledWith("DELETE", "/subscriptions/sub_123", {
      cancellation_details: { feedback: "too_expensive" },
    });
  });
  ...
  it("should_resume_subscription", async () => {
    ...
    expect(callApiSpy).toHaveBeenCalledWith("POST", "/subscriptions/sub_123/resume", { billing_cycle_anchor: "now" });
  });
  ...
  it("should_list_subscriptions_with_status_filter", async () => {
    ...
    expect(callApiSpy).toHaveBeenCalledWith("GET", "/subscriptions", { status: "active", limit: 25 });
  });

  it("should_handle_error_on_cancel", async () => {
    callApiSpy.mockRejectedValue(new StripeError(400, "invalid_request_error", undefined, "Cannot cancel"));
    const cb = registeredTools.get("cancel_subscription")!;
