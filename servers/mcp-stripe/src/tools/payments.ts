...
export function registerPaymentTools(server: McpServer, client: StripeClient): void {
  server.tool(
    ...
    async (p) =>
      withErrorHandler(async () =>
        toolResult(await client.callApi("POST", "/payment_intents", p as Record<string, unknown>)),
      ),
  );
  ...
  server.tool("list_payment_intents", "List PaymentIntents", ListPaymentIntentsSchema.shape, async (p) =>
    withErrorHandler(async () =>
      toolResult(await client.callApi("GET", "/payment_intents", p as Record<string, unknown>)),
    ),
  );
  ...
  server.tool("update_payment_intent", "Update a PaymentIntent", UpdatePaymentIntentSchema.shape, async (p) =>
    withErrorHandler(async () => {
      ...
      return toolResult(
        await client.callApi("POST", `/payment_intents/${encodeURIComponent(payment_intent_id)}`, params),
      );
    }),
  );
  ...
  server.tool("confirm_payment_intent", "Confirm a PaymentIntent", ConfirmPaymentIntentSchema.shape, async (p) =>
    withErrorHandler(async () => {
      const { payment_intent_id, ...params } = p;
      return toolResult(
        await client.callApi("POST", `/payment_intents/${encodeURIComponent(payment_intent_id)}/confirm`, params),
      );
    }),
  );
  ...
  server.tool(
    ...
    async (p) =>
      withErrorHandler(async () => {
        const { payment_intent_id, ...params } = p;
        return toolResult(
          await client.callApi("POST", `/payment_intents/${encodeURIComponent(payment_intent_id)}/capture`, params),
        );
      }),
  );
  ...
  server.tool("cancel_payment_intent", "Cancel a PaymentIntent", CancelPaymentIntentSchema.shape, async (p) =>
    withErrorHandler(async () => {
      const { payment_intent_id, ...params } = p;
      return toolResult(
        await client.callApi("POST", `/payment_intents/${encodeURIComponent(payment_intent_id)}/cancel`, params),
      );
    }),
  );

  server.tool("create_refund", "Refund a payment (full or partial)", CreateRefundSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("POST", "/refunds", p as Record<string, unknown>))),
  );
  ...
  server.tool("list_refunds", "List refunds", ListRefundsSchema.shape, async (p) =>
    withErrorHandler(async () => toolResult(await client.callApi("GET", "/refunds", p as Record<string, unknown>))),
  );
}
