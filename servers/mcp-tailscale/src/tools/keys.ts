...
export function registerKeyTools(server: McpServer, client: TailscaleClient) {
  ...
  server.tool(
    "create_auth_key",
    "Create a new auth key for device enrollment",
  ...
  server.tool(
    "delete_key",
    "Delete an auth key or API token by ID",
    KeyIdSchema.shape,
    async (p) =>
      withErrorHandler(async () => {
        await client.del(
          client.tailnetPath(`/keys/${encodeURIComponent(p.keyId)}`),
        );
