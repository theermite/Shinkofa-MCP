...
export const SetDeviceRoutesSchema = z.object({
  ...
  routes: z
    .array(z.string())
    .describe("List of CIDR routes to enable (e.g. ['10.0.0.0/24'])"),
});
...
export const CreateAuthKeySchema = z.object({
  reusable: z
    .boolean()
    .default(false)
    .describe("Can the key be reused to enroll multiple devices"),
  ephemeral: z
    .boolean()
    .default(false)
    .describe("Enroll devices as ephemeral (auto-removed when offline)"),
  preauthorized: z
    .boolean()
    .default(false)
    .describe("Pre-authorize enrolled devices (skip admin approval)"),
  tags: z
    .array(z.string())
    .optional()
    .describe("Tags to apply (e.g. ['tag:server']) — must exist in ACL"),
  expirySeconds: z
    .number()
    .int()
    .positive()
    .default(86400)
    .describe("Key validity in seconds (default 24h)"),
  description: z
    .string()
    .optional()
...
export const UpdateAclSchema = z.object({
  acl: z
    .string()
    .min(1)
    .describe("Full ACL document (HuJSON or JSON string)"),
  format: z
    .enum(["json", "hujson"])
    .default("hujson")
    .describe("ACL format; 'hujson' accepts comments"),
});

export const SetDnsNameserversSchema = z.object({
  dns: z
    .array(z.string())
    .describe("DNS server IPs for the tailnet (e.g. ['1.1.1.1'])"),
});
...
export const RawApiCallSchema = z.object({
  ...
  path: z
    .string()
    .describe("API path, e.g. /api/v2/tailnet/-/devices"),
  body: z
    .record(z.unknown())
    .optional()
