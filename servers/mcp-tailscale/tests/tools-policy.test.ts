...
beforeEach(() => {
  ...
  requestSpy = vi.spyOn(client, "request").mockResolvedValue("// ACL\n{}");
...
describe("Policy tools — registration", () => {
  it("should_register_all_5_policy_tools", () => {
    for (const name of ["get_acl", "update_acl", "get_dns_nameservers", "set_dns_nameservers", "get_dns_preferences"]) {
...
describe("Policy tools — calls", () => {
  it("should_get_acl_via_raw_request", async () => {
    ...
    await cb({});
    expect(requestSpy).toHaveBeenCalledWith("GET", "/api/v2/tailnet/ermite.ts.net/acl", undefined);
  });
  ...
  it("should_get_dns_nameservers", async () => {
    ...
    await cb({});
    expect(getSpy).toHaveBeenCalledWith("/api/v2/tailnet/ermite.ts.net/dns/nameservers");
  });

  it("should_set_dns_nameservers", async () => {
    const cb = registeredTools.get("set_dns_nameservers")!;
    await cb({ dns: ["1.1.1.1", "8.8.8.8"] });
    expect(postSpy).toHaveBeenCalledWith("/api/v2/tailnet/ermite.ts.net/dns/nameservers", {
      dns: ["1.1.1.1", "8.8.8.8"],
    });
  });
  ...
  it("should_get_dns_preferences", async () => {
    ...
    await cb({});
    expect(getSpy).toHaveBeenCalledWith("/api/v2/tailnet/ermite.ts.net/dns/preferences");
  });
});
