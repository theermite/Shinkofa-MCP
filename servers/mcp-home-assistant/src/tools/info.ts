...
export function registerInfoTools(server: McpServer, client: HAClient): void {
  ...
  server.tool("render_template", "Render a Jinja2 template (max 2000 chars, read-only state access)", { template: z.string().max(2000).describe("Jinja2 template string (max 2000 chars)") }, async (p) => {
  ...
  server.tool("get_logbook", "Get logbook entries", { timestamp: z.string().optional().describe("ISO 8601 start time"), entity_id: z.string().optional().describe("Filter by entity"), end_time: z.string().optional().describe("ISO 8601 end time") }, async (p) => {
    return withErrorHandler(async () => {
  ...
  server.tool("get_calendar_events", "Get events from a calendar", { entity_id: z.string(), start: z.string().describe("ISO 8601 start time (required by HA)"), end: z.string().describe("ISO 8601 end time (required by HA)") }, async (p) => {
    return withErrorHandler(async () => {
