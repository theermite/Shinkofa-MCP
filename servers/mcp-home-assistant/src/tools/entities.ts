...
export function registerEntityTools(server: McpServer, client: HAClient): void {
  server.tool(
    "list_states",
    "List all entity states (can be 300+ items)",
  ...
  server.tool(
    "set_state",
    "Set/update an entity state record (WARNING: does NOT control physical devices — use call_service for that)",
    {
      entity_id: EntityId,
    ...
    async (p) => {
      return withErrorHandler(async () =>
        toolResult(
          await client.callApi("POST", `/states/${p.entity_id}`, { state: p.state, attributes: p.attributes }),
        ),
      );
    },
  );
  ...
  server.tool(
    "call_service",
    "Call a Home Assistant service. Common: domain='light' service='turn_on', domain='scene' service='turn_on', domain='automation' service='trigger', domain='script' service='turn_on', domain='climate' service='set_temperature'",
    {
      domain: z.string().describe("Service domain (light, switch, scene, etc.)"),
      service: z.string().describe("Service name (turn_on, turn_off, toggle, etc.)"),
      entity_id: EntityId.optional().describe("Entity to target (e.g. 'light.living_room')"),
  ...
  server.tool(
    ...
    "Fire a custom event",
    {
      event_type: z.string().describe("Event type to fire (e.g. 'custom_event')"),
      event_data: z.record(z.unknown()).optional().describe("Event payload data"),
    },
    async (p) => {
      return withErrorHandler(async () =>
        toolResult(await client.callApi("POST", `/events/${p.event_type}`, p.event_data)),
      );
    },
  );
}
