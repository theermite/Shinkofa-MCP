...
export const GetEnvVarsSchema = z.object({
  filter: z
    .string()
    .optional()
    .describe("Case-insensitive substring filter on variable names"),
  unmask: z
    .boolean()
    .default(false)
    .describe(
      "Unmask values of secret-looking vars (TOKEN/KEY/SECRET/...). Default false.",
    ),
});
...
export const ListProcessesSchema = z.object({
  limit: z
    ...
    .max(500)
    .default(20)
    .describe("Max processes to return (default 20, max 500)"),
  sortBy: z
    .enum(["cpu", "memory", "pid"])
    .default("cpu")
    .describe("Sort key"),
...
export const ExecCommandSchema = z.object({
  ...
  args: z
    .array(z.string())
    .default([])
    .describe("Arguments passed to executable"),
  ...
  env: z
    .record(z.string())
    .optional()
    .describe("Additional env vars (merged on process.env)"),
});
