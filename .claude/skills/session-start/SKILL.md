...
6. **BLUEPRINT CHECK** *(SKIP in LITE_MODE)*: Verify `docs/Blueprint.md` exists and is current. If the project has `docs/critical-paths.md`, load it — risk classification affects all quality gates during the session.
7. **CDC CHECK** *(SKIP in LITE_MODE)*: Verify `docs/CDC.md` exists. Flag any drift from implementation.
